...
The four-database matrix includes SELECT compatibility plus INSERT/UPDATE/DELETE native affected counts, zero-row success, MCP mutation, constraint normalization, startup EXPLAIN behavior, persisted DB state, and the `TestContainerDBDriverMutationReconciliation` lost-response/reconciliation scenarios using the example schema and capability templates with REST/MCP requests. Container-required gates must use `ONPREST_IT_REQUIRE_CONTAINERS=1`; a skipped DB is not a release pass.
