...
Normal Agent runtime detail uses the executable path plus `.log`, for example `onprest-agent.log`. Standalone validation never opens that rotating file; a successfully recorded validation diagnostic returns its separate path beside the binary. See [Agent Local Logs](/agent/local-logs) for the authoritative lifecycle.
