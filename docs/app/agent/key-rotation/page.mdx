...
4. In one maintenance window, restart gateway with the new public key.
5. Immediately restart agent with the matching private key.
...
The signature is also bound to a fresh WebSocket handshake key, and there is no legacy-signature fallback. Roll back the gateway and agent binaries as a pair if a release rollback is required.
