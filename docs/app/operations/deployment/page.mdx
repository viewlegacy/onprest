...
`service remove` is an alias for `service uninstall`. `service install` can omit `--config`; in that case it registers `capability.yaml` next to the `onprest-agent` binary. It does not enable automatic start on OS boot. Configure boot-time startup separately with the host OS if required.
