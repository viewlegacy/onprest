...
Gateway and agent versions that use the handshake-bound signature must be deployed as one coordinated release. The old signature format is deliberately not accepted. Stop or drain callers, update gateway and agent configuration/binaries in the same maintenance window, then restart gateway followed by agent and run the verification above. Rollback must likewise restore both binaries together; do not leave a mixed-version pair running.
...
`service remove` is a compatible alias for `service uninstall`. `service install` can omit `--config`; in that case it registers `capability.yaml` next to the `onprest-agent` binary. It does not enable automatic start on OS boot. Configure boot-time startup separately with the host OS if required.
