...
# Add --build-arg VERSION=1.2.4 to both commands for a release build.
```

The Docker builder uses the same `VERSION` value as `make build`; an omitted or empty value is reported as `dev` by the binary and MCP `initialize` metadata.
