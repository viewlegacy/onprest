...
Database TLS CA and client certificate files are loaded when the Agent creates the DB pool. Coordinate their rotation with the database server, replace each Agent-side file atomically, run `onprest-agent validate --config PATH`, and restart the Agent through the host service manager so a new pool loads the new material. Confirm a capability call after restart before retiring the old trust material. A failed CA, expiry, or hostname check stops the connection; the Agent does not retry with a weaker mode or plaintext.
