...
The HTTP request body limit is 1 MiB (1,048,576 bytes). An oversized body, malformed JSON, or multiple top-level JSON values produces a JSON-RPC parse error (`-32700`, `PARSE_ERROR`) with HTTP 200. The gateway also bounds body-read time.
