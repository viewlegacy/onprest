...
The agent checks whether the capability exists, whether params match the contract, and whether policy is satisfied before querying the DB. Read-only capabilities accept exactly one top-level `SELECT`; integer JSON tokens remain exact through signed 64-bit conversion; response values must be finite and representable by the declared result type.
...
Database TLS is independent from gateway-edge TLS. PostgreSQL can use `require`, `verify-ca`, or `verify-full` (and optional client certificates); SQL Server can use `require` or `verify-full`. `disable` remains the compatibility default. Unsupported MySQL/Oracle TLS requests are rejected instead of silently downgraded.
