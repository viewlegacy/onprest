...
Agent connections are limited to one live connection. The gateway sends WebSocket ping frames and requires timely pong traffic, so a stale TCP session is closed and a replacement may connect. If another agent tries to connect while the existing session is still live, the gateway rejects it and writes a warning log.
...
- up to `runtime.max_concurrent_requests` concurrent capability executions per connection (default 16)
...
Request IDs correlate responses, so concurrent responses may arrive out of request order. WebSocket writes are serialized at the frame boundary. The transport handles fragmentation, ping/pong/close control frames, mask-direction validation, bounded messages, and handshake deadlines.
