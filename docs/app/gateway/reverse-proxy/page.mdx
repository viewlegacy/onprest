...
A reverse proxy or load balancer is optional. Use this configuration when TLS or the public network edge is handled outside `onprest-gateway`; no specific proxy product is required.
...
| `/ws/agent/challenge` | HTTP POST | one-time challenge shared by normal Agent authentication and `doctor` |
| `/ws/agent/verify` | HTTP POST | `doctor` signature verification |
...
The `/ws/agent` route must support WebSocket upgrade. The challenge and verify routes are ordinary POST requests and do not use API-key authentication or WebSocket upgrade. Challenge issuance requires no Agent signature; verify requires the Agent's Ed25519 signature.
...
`GATEWAY_IP_ALLOW_LIST` applies after source IP detection to REST, MCP, OpenAPI, `/healthz`, the agent WebSocket route, and the Agent diagnostic routes. Per-source rate limiting applies to the same routes. `/healthz` and Agent diagnostics are API-key-free, not exempt from these network controls.
