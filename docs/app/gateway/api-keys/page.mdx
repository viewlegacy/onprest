...
The Gateway accepts at most 100 configured API keys. It stores key hashes, not
plaintext keys. If authentication is temporarily busy, a caller may receive
HTTP 429 `GATEWAY_RATE_LIMITED`; retry later. Restart the Gateway after key
