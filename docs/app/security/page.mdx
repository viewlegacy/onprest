...
During each WebSocket handshake, the gateway verifies an Ed25519 signature over this exact UTF-8 byte sequence (with LF separators and no trailing LF):
...
The version prefix prevents cross-protocol reuse, and binding the signature to the handshake's fresh `Sec-WebSocket-Key` prevents a captured signature from authorizing another handshake. The gateway validates the signature before atomically consuming the nonce. Timestamp tolerance and nonce reuse rejection add defense in depth. There is no compatibility path for the former signature format; gateway and agent must be upgraded together.
