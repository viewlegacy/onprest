...
The version prefix prevents cross-protocol reuse, and binding the signature to the handshake's fresh `Sec-WebSocket-Key` prevents a captured signature from authorizing another handshake. The gateway validates the signature before atomically consuming the nonce. Timestamp tolerance and nonce reuse rejection add defense in depth.
