...
Onprest's capability model fits AI/MCP because the cloud-facing side is intentionally not trusted with business meaning. The gateway can authenticate callers and route capability calls, but the on-prem agent owns SQL, parameter validation, execution policy, SELECT output filtering, and DML affected-count validation. AI agents and MCP clients see only the capabilities their API key is allowed to list and call.
...
| `onprest-agent` | `capability.yaml` loading, DB ping, EXPLAIN, param/policy validation, SELECT result filtering, DML count validation, and SQL execution |
...
- [Capability YAML](/agent/capability-yaml): Define inputs and policy, plus SELECT output allow-lists; DML is count-only.
