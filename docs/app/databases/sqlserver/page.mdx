...
`verify-ca` and client certificate/key fields are rejected for SQL Server. Existing files with no `tls` section retain `encrypt=disable`. Reconnects retain the configured TLS mode and never retry in plaintext. To replace a private CA, follow the [database TLS rotation procedure](/operations/deployment#database-tls-certificate-rotation).
...
The release suite uses a real TDS connection to verify encryption, certificate and hostname failures, certificate replacement, and reconnect behavior. See [Test Commands](/reference/test-commands) for the complete release-test scope.
